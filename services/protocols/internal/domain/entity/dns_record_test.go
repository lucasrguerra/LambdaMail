...
// BIMI is published only when there is a mark to point at.
//
// A record that advertises a logo which is not there is worse than no record:
// a receiver stops looking once it finds one, so an empty pointer buys a
// failed fetch instead of the fallback it would otherwise have used.
func TestBuildDnsRecordSpecs_BimiOnlyWhenThereIsALogo(t *testing.T) {
	without := BuildDnsRecordSpecs(DnsRecordSpec{
		DomainName: "example.test",
		MailHost:   "mail.example.test",
		ServerIPv4: "192.0.2.1",
	})
	for _, r := range without {
		if strings.Contains(r.Name, "_bimi") {
			t.Errorf("published %s with no logo configured", r.Name)
		}
	}

	with := BuildDnsRecordSpecs(DnsRecordSpec{
		DomainName:  "example.test",
		MailHost:    "mail.example.test",
		ServerIPv4:  "192.0.2.1",
		BimiLogoURL: "https://mail.example.test/.well-known/bimi/default.svg",
	})

	var found *DnsRecord
	for i, r := range with {
		if r.Name == "default._bimi.example.test" {
			found = &with[i]
		}
	}
	if found == nil {
		t.Fatal("no BIMI record was published for a domain with a logo")
	}
	if found.Type != "TXT" {
		t.Errorf("BIMI record type is %q, want TXT", found.Type)
	}
	if !strings.HasPrefix(found.Value, "v=BIMI1;") {
		t.Errorf("value %q does not start with the version tag", found.Value)
	}
	// No certificate configured, so no a= tag: an empty one would advertise a
	// certificate that cannot be fetched.
	if strings.Contains(found.Value, "a=") {
		t.Errorf("value %q carries an empty certificate tag", found.Value)
	}
}

// Gmail and Outlook render the mark only when the record points at a Verified
// Mark Certificate, so the tag has to be carried when there is one.
func TestBuildDnsRecordSpecs_BimiCarriesTheCertificateWhenConfigured(t *testing.T) {
	specs := BuildDnsRecordSpecs(DnsRecordSpec{
		DomainName:  "example.test",
		MailHost:    "mail.example.test",
		ServerIPv4:  "192.0.2.1",
		BimiLogoURL: "https://mail.example.test/.well-known/bimi/default.svg",
		BimiVmcURL:  "https://mail.example.test/.well-known/bimi/default.pem",
	})
	...
	for _, r := range specs {
		...
		if !strings.Contains(r.Value, "a=https://mail.example.test/.well-known/bimi/default.pem") {
			t.Errorf("value %q does not carry the certificate", r.Value)
		}
		return
	}
	t.Fatal("no BIMI record was published")
}
