...
func (r *MessageQueryRepository) ListMessages(ctx context.Context, folderID string) ([]port.MessageRecord, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT m.uid, m.blob_id, m.size_bytes, m.received_at,
		...
		GROUP BY m.uid, m.blob_id, m.size_bytes, m.received_at
		ORDER BY m.uid ASC
	`, folderID)
	if err != nil {
		return nil, err
	...
	for rows.Next() {
		...
		if err := rows.Scan(&rec.UID, &rec.BlobID, &rec.SizeBytes, &rec.ReceivedAt, &rec.Flags); err != nil {
			return nil, err
		}
		out = append(out, rec)
	}
